...
\begin{oobs}
    ...
    \begin{deff}
      ...
      \begin{oobs}\label{provaPerelman}
        ...
        \begin{oobs}
        A conjectura da geometrização de Thurston está relacionada à física, no problema de determinar a topologia do Universo. Mais detalhes podem ser encontrados em \mycitep{passeio}.
        \end{oobs}
  ...
  \begin{demm}
    Consulte \mycitep{MinhaDissertacao}.
  \end{demm}
...
\end{align}
It turns out (for details, see \mycitep{DensoOverflow}) that in $\mm_{\mathcal{A}}$ the eigenvalues of $\mathcal{A}$ are well defined and define smooth functions \[\lambda_{1},\ldots,\lambda_{n}:\mm_{\mathcal{A}}\to\mathbb{R}.\] Furthermore, he proved that in such a set the following is true
...
\begin{teorema}\label{decompwarp}
    ...
    \begin{oobs}
        ...
        Now we obtain a local description of $M$, which is a consequence of Lemma \ref{lemmamany} and Theorem \cref{maxxnumeig-INTRO}. First, recall the following formulas for the Ricci and scalar curvatures of a multiply warped product (whose proofs can be found, for instance, in \mycitep{provaWarped}):\\
        ...
        \end{demm}
        \begin{oobs}
            The previous theorem can be compared to Garcia-Rio's result \mycitep{GarciaRio}, namely,
...
\begin{teorema}[Local multiply warped product structure of harmonic gradient almost solitons]\label{localstrucHarm}
	...
	\begin{enumerate}[label=\color{blue}\normalfont\textbf{(\theenumi)}]
		...
		\]
	\end{enumerate}
	 \MH{Furthermore, $I$ is the integral manifold of $\partial_{1}=\frac{\nabla f}{|\nabla f|}$ and $\lambda,\lambda_{1},\ldots,\lambda_{n},R$ and $|\nabla f|$ restricted to $U$ depend only on $x_{1}=s\in I$. \\}
	 
	 
	 \MH{
	 \begin{demm}
	 Suppose $\Ric$ has exactly two eigenvalues and $\Rm$ is harmonic. Clearly, we are then in case \cref{simplesHarm}. Multiplying the last equation of \cref{SistemaSimples} by $(n-1)$ and using \cref{ScalWarped}, we obtain
	 \[ \begin{cases}
	 \tau_{\nn}\left(1 - \frac{\lambda_2}{\lambda_{\nn}} h^2\right) - (n-1) h h'' - (n-1)(n-2)(h')^2 = 0 \\
	  \tau_{\nn} - 2(n-1)hh''- (n-1)(n-2)(h')^2 = \tau_{\mm} \cdot h^2 
	 \end{cases}
	 \]
	 which, as desired, imply
	 \[
	 h'' = \frac{(\tau_{\nn} \frac{\lambda_2}{\lambda_{\nn}} - \tau_{\mm})}{n-1} h,
	 \]
	 where
	 \[
	\lambda_2 = \lambda - f' \frac{h'}{h}.
	 \]
	On the other hand, if $\Ric$ has three eigenvalues, in an entirely analogous manner, lemma \cref{warped} yields
	\[
		\begin{cases}
			\begin{aligned}
			&0 = -\tau_{\mm} (h_1)^2 -2 r_1 h_1 (h_1)''- 2 r_2 \frac{(h_2)''}{h_2} (h_1)^2  +  \tau_{\nn_1} + \tau_{\nn_2} \frac{(h_1)^2}{(h_2)^2} \\
			 &- r_1 (r_1 - 1)(h_1')^2 - r_2 (r_2 - 1) \frac{(h_2')^2}{(h_2)^2} (h_1)^2 - r_1 r_2 h_1 h_1' \frac{h_2'}{h_2}\\
			\end{aligned} \vspace{0.3cm} \\
			0 = -\tau_{\nn_1}\left(1 - \frac{\lambda_2}{\lambda_{\nn_1} }  (h_1)^2 \right) + r_1 h_1 h_1 '' + r_1 (r_1 - 1) (h_1')^2 + r_1 r_2 h_1 h_1' \frac{h_2'}{h_2} 
			\end{cases}
	\]
	which implies
	\[
		\tau_{\mm} =  -   r_1 \frac{ (h_1)''}{h_1} - 2 r_2 \frac{(h_2)''}{h_2}  +  \frac{\tau_{\nn_2}}{(h_2)^2} - r_2 (r_2 - 1) \frac{(h_2')^2}{(h_2)^2} 
	\]
	Similarly, from
	\[
		\begin{cases}
			\begin{aligned}
			&0 = -\tau_{\mm} (h_2)^2 -2 r_1 h_1 (h_1)''- 2 r_2 \frac{(h_2)''}{h_2} (h_1)^2  +  \tau_{\nn_1} + \tau_{\nn_2} \frac{(h_1)^2}{(h_2)^2} \\
			 &- r_1 (r_1 - 1)(h_1')^2 - r_2 (r_2 - 1) \frac{(h_2')^2}{(h_2)^2} (h_1)^2 - r_1 r_2 h_1 h_1' \frac{h_2'}{h_2}\\
			\end{aligned} \vspace{0.3cm} \\
			0 = -\tau_{\nn_2}\left(1 - \frac{\lambda_3}{\lambda_{\nn_2} }  (h_2)^2 \right) + r_2 h_2 h_2 '' + r_2 (r_2 - 1) (h_2')^2 + r_1 r_2 h_2 h_2' \frac{h_1'}{h_1}
			\end{cases}
	\]
	we get
	\[
		\tau_{\mm}= -   r_2 \frac{(h_2)''}{h_2} - 2 r_1 \frac{(h_1)''}{h_1} + \frac{\tau_{\nn_1} }{(h_1)^2} - r_1 (r_1 - 1) \frac{(h_1')^2}{(h_1)^2} 
	\]
        \end{demm}
...
\begin{oobs}
    ...
    \begin{lema}\label{divrm}
       ...
       Basta provarmos que
        ...
        \begin{proposicao}\label{CaractRmHarmonico} Em dimensão $\geq 4$, 
        \[
...
\bibitem{provaWarped}
\bl{
\textbf{Horácio, M.A.R.M.} \emph{Produtos warped}. \\ Disponível em: \href{https://sagangromov.github.io/assets/pdf/OneChart.pdf}{\textbf{https://sagangromov.github.io/assets/pdf/ProdutosWarped1.pdf}}. 
}

\bibitem{passeio}
\bl{
\textbf{Horácio, M.A.R.M.} \emph{Um passeio histórico: de Eratóstenes a Perelman e a topologia do Universo}. \\ Disponível em: \href{https://sagangromov.github.io/assets/pdf/OneChart.pdf}{\textbf{https://sagangromov.github.io/assets/pdf/passeio.pdf}}. 
}
...
\bl{{MathOverflow;} \href{https://mathoverflow.net/q/433698/119418}{\textbf{What are some explicit examples of nontrivial gradient almost Ricci solitons with harmonic curvature?}} Disponível em:\\ \href{https://mathoverflow.net/q/433698/119418}{\textbf{https://mathoverflow.net/q/433698/119418}}. 
}



\bibitem{DensoOverflow}
\bl{{MathOverflow;} \href{https://mathoverflow.net/q/433698/119418}{\textbf{Why is this subset associated to a 2-tensor dense?}} \\Disponível em:\\ \href{https://mathoverflow.net/questions/453659/why-is-this-subset-associated-to-a-2-tensor-dense
}{\textbf{https://mathoverflow.net/questions/453659/why-is-this-subset-associated-to-a-2-tensor-dense
}}.
}
